            ...
            <a-row :gutter="16">
              <a-col :span="12">
                <a-form-item
                  ...
                  <a-radio-group @change="switchJobType" v-decorator="['collectionType',{rules: [{ required: true, message: '请选择全量或者增量'}],'initialValue':0}]">
                    <a-radio :value="0">全量</a-radio>
                    <a-radio :value="1">增量</a-radio>
                  </a-radio-group>
                </a-form-item>
              </a-col>
              <a-col :span="12">
...
<!--                  <a-col>-->
                    ...
                    <div style="display: flex;text-align: left;border-left: 1px solid #cccccc;flex-wrap:wrap">
                      <div v-for="(item,index) in originalTableColumns" :key="index" v-if="item.incColumn===1" v-model="incColumn" style="width: 250px;" @click="confirmIncColumn(item.columnName)">
                        <div class="incColumn">
                          <div style="display: flex">
                            <div style="width: 220px;">{{item.columnName}}&nbsp;<span style="color: #cccccf">({{item.columnType}})</span></div>
                            <div><a-icon v-if="incColumn===item.columnName" style="color: #00DB00" type="check-circle"/></div>
                          </div>
                        </div>
<!--                        <div v-if="incColumn===item.columnName" class="incColumnChecked">-->
<!--                          {{item.columnName}}&nbsp;<span style="color: #cccccf">({{item.columnType}})</span>-->
<!--                        </div>-->
                      </div>
                    </div>
<!--                  </a-col>-->
<!--                </a-row>-->
              ...
              <div style="width: 1200px;">
                <div v-if="originalTableColumns.length > 0"
                  ...
                  <div v-if="originalTableColumns.length>0 && targetTableColumns.length > 0">
                    <a-button type="primary" style="margin-right: .8rem">自动匹配</a-button>
                  </div>
          ...
          <div v-if="item.key === 5" class="steps-content">5</div>

        </a-collapse-panel>
      </a-collapse>

      <div style="margin-top: 15px;text-align: right;">
        <a-popconfirm title="确定放弃编辑？" okText="确定" cancelText="取消" @confirm="confirmCancel">
